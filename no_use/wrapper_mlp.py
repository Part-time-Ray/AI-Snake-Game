...
class environment(gym.Env):
    def __init__(self, seed=0, size=12, training_mode=True, barrier_mode=True):
        ...
        self.size = size
        self.observation_space = gym.spaces.Box(low=-1, high=3, shape=(self.size, self.size), dtype=np.float32)
        self.done = None
        self.step_cnt = 0
    def reset(self):
        self.game.reset()
    ...
    def step(self, action):
        ...
        reward = 0
        self.step_cnt += 1
        if self.step_cnt>=self.size*self.size*5: #too much setp
            self.done = True
        if self.done:
            ...
            self.step_cnt = 0
            reward += math.exp(info['snake_len']/self.size**2)*2 # (e^(3/144)*2==2.04, e*2==5.42)
        else:
        ...
        reward += (-self.entropy(zone)+0.08)*(info['snake_len']/self.size**2)*5 # (-12.5, 0.4)
    ...
    def get_obs(self):
        ...
        re[tuple(np.array(self.game.snake).T)] = np.linspace(3, 2, len(self.game.snake), dtype=np.float32)
        if self.game.barrier_mode and len(self.game.barrier):
            re[tuple(np.array(self.game.barrier).T)] = -1
        re[tuple(np.array(self.game.food).T)] = 1
        return re
    def render(self):
        self.game.render()
    def close(self):
